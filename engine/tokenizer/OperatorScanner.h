...
inline Token scanOperator(ScanState& s) {
    ...
    if (c == '>') {
        s.advance();
        if (s.peek() == '=') { s.advance(); return Token(TokenType::GE, ">=", start, s.pendingSpaceBefore); }
        return Token(TokenType::GT, ">", start, s.pendingSpaceBefore);
    }
    if (c == '<') {
        s.advance();
        if (s.peek() == '=') { s.advance(); return Token(TokenType::LE, "<=", start, s.pendingSpaceBefore); }
    ...
    case '!': return make(TokenType::BANG, "!");
