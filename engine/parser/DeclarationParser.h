...
public:
    ...
    static bool isFunctionDecl(ParserCore& p) {
        if (!p.check(TokenType::SET)) return false;
        return p.peek(1).is(TokenType::FUNCTION)
            || p.peek(1).is(TokenType::RETURNABLE)
            || p.peek(1).is(TokenType::ASYNC);
    }
    ...
    static std::unique_ptr<Stmt> parseSet(ParserCore& p) {
        ...
        // Parse type keyword
        std::string varType;
        if (p.check(TokenType::NUMBER_TYPE)) { varType = "number"; p.advance(); }
        else if (p.check(TokenType::STR_TYPE)) { varType = "str"; p.advance(); }
        else if (p.check(TokenType::LIST_TYPE)) { varType = "list"; p.advance(); }
        else if (p.check(TokenType::MAP_TYPE)) { varType = "map"; p.advance(); }
        else if (p.check(TokenType::BOOLEAN_TYPE)) { varType = "boolean"; p.advance(); }
        else if (p.check(TokenType::EMPTY)) { varType = "empty"; p.advance(); }
        else {
